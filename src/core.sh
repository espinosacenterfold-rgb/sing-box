...
change() {
    ...
    10)
        ...
        [[ $(grep -i "^example.com$" <<<$is_servername) ]] && {
            err "你干嘛～哎呦～"
        }
        add $net
        ;;
    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen && ! $is_dont_test_host ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m no-auto-tls 帮助(help)\e[0m: $(msg_ul https://github.com/espinosacenterfold-rgb/sing-boxs_core/no-auto-tls/)\n"
...
get() {
    ...
    protocol)
        ...
        socks*)
            net=socks
            is_protocol=$net
            [[ ! $is_socks_user ]] && is_socks_user=singbox
...
info() {
    ...
    ws | tcp | h2 | quic | http*)
        if [[ $host ]]; then
            ...
            [[ $is_protocol == 'vmess' ]] && {
                is_vmess_url=$(jq -c '{v:2,ps:'\"upstream-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
            ...
            is_vmess_url=$(jq -c "{v:2,ps:\"upstream-${net}-$is_addr\",add:\"$is_addr\",port:\"$port\",id:\"$uuid\",aid:\"0\",net:\"$net\",type:\"$is_type\"$is_quic_add}" <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        fi
        ;;
    ss)
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装请查看脚本帮助文档: $(msg_ul https://github.com/espinosacenterfold-rgb/sing-boxs_core/$is_core-script/)"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
    ...
    if [[ $is_no_auto_tls ]]; then
        ...
        msg "端口(port): $port"
        msg "路径(path): $path"
        msg "\e[41m帮助(help)\e[0m: $(msg_ul https://github.com/espinosacenterfold-rgb/sing-boxs_core/no-auto-tls/)"
    fi
    footer_msg
}
...
footer_msg() {
    ...
    msg "------------- END -------------"
    msg "关注(tg): $(msg_ul )"
    msg "文档(doc): $(msg_ul https://github.com/espinosacenterfold-rgb/sing-boxs_core/$is_core-script/)"
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            link="https:///tools/qr.html#${is_url}"
...
is_main_menu() {
    ...
    msg "群组(Chat): $(msg_ul https://t.me/tgupstream)"
    is_main_start=1
    ask mainmenu
    case $REPLY in
    1)
